...
// captures default microphone (or communications device), converted to 48kHz/stereo/float32
// make sure CoInitializeEx has been called before calling this
static bool AudioCapture_StartMicrophone(AudioCapture* Capture);
...
// shared tail: allocates ringbuffer, starts capture thread and the audio client
// used by all three capture modes (process-loopback, system-loopback, microphone)
static bool AudioCapture__StartCommon(AudioCapture* Capture)
{
...
bool AudioCapture_Start(AudioCapture* Capture, HWND ApplicationWindow)
{
    ...
    if (Result)
    {
        AudioCapture__StartCommon(Capture);
    }

    return Result;
}

// Captures default microphone / communications input device.
// Forced to 48kHz / stereo / IEEE float, same as process-loopback path above,
// so it can be mixed sample-for-sample with the game+Discord loopback stream
// in wcap_audio_mixer.h without any resampling step.
bool AudioCapture_StartMicrophone(AudioCapture* Capture)
{
    IMMDeviceEnumerator* Enumerator;
    HR(CoCreateInstance(&CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL, &IID_IMMDeviceEnumerator, (void**)&Enumerator));

    IMMDevice* Device;
    // prefer the "communications" default (this is what Discord itself defaults to),
    // fall back to the console default microphone if that role isn't set
    if (FAILED(IMMDeviceEnumerator_GetDefaultAudioEndpoint(Enumerator, eCapture, eCommunications, &Device)))
    {
    ...
    IAudioClient* Client;
    if (FAILED(IMMDevice_Activate(Device, &IID_IAudioClient, CLSCTX_ALL, NULL, (void**)&Client)))
    {
        IMMDevice_Release(Device);
        IMMDeviceEnumerator_Release(Enumerator);
        return false;
    ...
    // AUTOCONVERTPCM lets WASAPI resample/upmix whatever the mic's native format is
    // (typically 16-bit/48kHz/mono) into our fixed float/stereo/48kHz target
    DWORD Flags = AUDCLNT_STREAMFLAGS_AUTOCONVERTPCM | AUDCLNT_STREAMFLAGS_SRC_DEFAULT_QUALITY | AUDCLNT_STREAMFLAGS_EVENTCALLBACK;
    HRESULT hr = IAudioClient_Initialize(Client, AUDCLNT_SHAREMODE_SHARED, Flags, MF_UNITS_PER_SECOND, 0, &FormatEx->Format, NULL);
    if (FAILED(hr))
    {
    ...
    return AudioCapture__StartCommon(Capture);
}
